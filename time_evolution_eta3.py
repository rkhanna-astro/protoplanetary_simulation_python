...
class DiskAnalysisEta3:
    ...
    def _process_single_case(self, case: str, pf_case: np.ndarray, minr: float, rmid: float):
        ...
        self.results[f'Mdisk_{prefix}'].append(pf_case[6][-1])
        self.results[f'Ewind_{prefix}'].append(pf_case[7][-1])
        self.results[f'j_{prefix}'].append(pf_case[1][-1] * pf_case[2][-1] * pf_case[3][-1])
        ...
        self.results[f'M_tot_{prefix}'].append(M_tot)
        self.results[f'J_tot_{prefix}'].append(J_tot)
        
        # Mass envelope calculations
        MdMenv = pf_case[10][-1]
        Menv = MdMenv / pf_case[6][-1]
        
        self.results[f'MdMenv_{prefix}'].append(MdMenv)
        self.results[f'Menv_{prefix}'].append(Menv)

def main():
    # Load your data - replace with actual data loading
    pf_dict_eta3 = {}    # Should contain pf1_pre, pf1, pf1_post, pf2_pre, etc.
    pf_dict_eta3nw = {}  # No-wind version
    
    # Get minimum radius from one of the profiles
    minr = pf_dict_eta3['pf1'][1][0] if 'pf1' in pf_dict_eta3 else 0
    
    # Process with wind
    print("Processing eta3 with wind...")
    start_time = time.time()
    eta3_analysis = DiskAnalysisEta3()
    results_eta3 = eta3_analysis.process_time_series(pf_dict_eta3, minr)
    print(f"Completed in {time.time() - start_time:.2f} seconds")
    
    # Process no wind case
    print("\nProcessing eta3 no wind case...")
    start_time = time.time()
    eta3nw_analysis = DiskAnalysisEta3()
    results_eta3nw = eta3nw_analysis.process_time_series(pf_dict_eta3nw, minr)
    print(f"Completed in {time.time() - start_time:.2f} seconds")
    
    return results_eta3, results_eta3nw

if __name__ == "__main__":
    results_eta3, results_eta3nw = main()
