...
def bb():
    ...
    sol = solve_bvp(deriv, bcs, solinit[0], solinit[1])
...
def deriv(x, Y):
    ...
    dYdx = np.zeros((3, Y.shape[1]))
    dYdx[0] = (2.0 * (Y[0])**2 / (alpha * x**(l - 1.0)) +
               3.0 * (2.0 * l - 1) * Y[0]**2 +
               (6.0 * (l + 2.0) * gamma - 8.0 * (2.0 * l + 1.0)) * Y[0] * x +
               (4.0 * (2.0 * l + 3.0) - 4.0 * (l + 3.0) * gamma) * x**2.0) / (2.0 * x * (3.0 * Y[0] - (3.0 * gamma - 2.0) * x))
