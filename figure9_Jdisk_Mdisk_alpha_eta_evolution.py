...
def run_simulation(result_key):
    ...
    while tps <= 3.e3:
        ...
        pf_dict[f'pf{ii}_{alpha_0}_{etaprime}'] = process.process(tps, x_sh_test, gamma_eff, alpha_0, etaprime, Mdot_stable)
        print("Properties2", alpha_0, etaprime, gamma_eff, Mdot_stable)
...
for key in results.keys():
    ...
    for ii in range(1, 42):
        ...
        print("dr", etaprime, dr)

        if alpha == 0.3 and etaprime == 0.1:
            J_tot_pre_eta1.append(J_tot)
            M_tot_pre_eta1.append(M_tot)
        
        if alpha == 0.3 and etaprime == 0.01:
            J_tot_pre_eta2.append(J_tot)
            M_tot_pre_eta2.append(M_tot)
        
        if alpha == 0.3 and etaprime == 0.001:
            J_tot_pre_eta3.append(J_tot)
            M_tot_pre_eta3.append(M_tot)
        ...
        if alpha == 0.5 and etaprime == 0.1:
            J_tot_eta1.append(J_tot)
            M_tot_eta1.append(M_tot)
        
        if alpha == 0.5 and etaprime == 0.01:
            J_tot_eta2.append(J_tot)
            M_tot_eta2.append(M_tot)
        
        if alpha == 0.5 and etaprime == 0.001:
            J_tot_eta3.append(J_tot)
            M_tot_eta3.append(M_tot)
        
        if alpha == 0.8 and etaprime == 0.1:
            J_tot_post_eta1.append(J_tot)
            M_tot_post_eta1.append(M_tot)
        
        if alpha == 0.8 and etaprime == 0.01:
            J_tot_post_eta2.append(J_tot)
            M_tot_post_eta2.append(M_tot)

        if alpha == 0.8 and etaprime == 0.001:
            J_tot_post_eta3.append(J_tot)
            M_tot_post_eta3.append(M_tot)
...
jspec_etavar_g.jspec_etavar(
        np.array(time_evolution), np.array(J_tot_pre_eta1), np.array(M_tot_pre_eta1), np.array(J_tot_eta1), np.array(M_tot_eta1), np.array(J_tot_post_eta1), np.array(M_tot_post_eta1),
        np.array(time_evolution), np.array(J_tot_pre_eta2), np.array(M_tot_pre_eta2), np.array(J_tot_eta2), np.array(M_tot_eta2), np.array(J_tot_post_eta2), np.array(M_tot_post_eta2),
        np.array(time_evolution), np.array(J_tot_pre_eta3), np.array(M_tot_pre_eta3), np.array(J_tot_eta3), np.array(M_tot_eta3), np.array(J_tot_post_eta3), np.array(M_tot_post_eta3)
)
print(check_array)
