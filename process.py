...
def process(tps, x_sh_test, gamma_eff, alpha_0, etaprime, Mdot_stable):
    ...
    s0_root = fsolve(funbvps, [s0_i, s0_f])[0]
    print(f's0_root = {s0_root} | f(s0_root) = {funbvps(s0_root)}')

    x = np.linspace(x0, xf, 100)
    Yv = odeint(funsys, [globals.v0, s0_root, globals.w0, globals.G0, globals.E0], x)
    ...
    plotmat = makedata(x, Yv, globals.ts, Menv0)
