...
class NoWindAnalysis:
    ...
    def process_time_series(self, pf_dict: Dict, minr: float, 
        ...
        tps = initial_tps
        ii = 1
        rmid = 3000  # Middle radius for calculations
...
def main():
    ...
    pf_dict_nw = {}  # Should contain pf1_pre, pf1, pf1_post, pf2_pre, etc.
    
    # Get minimum radius from one of the profiles
    minr = pf_dict_nw['pf1'][1][0] if 'pf1' in pf_dict_nw else 0
    
    # Process no wind case
    print("Processing no wind case...")
    start_time = time.time()
    nw_analysis = NoWindAnalysis()
    results_nw = nw_analysis.process_time_series(pf_dict_nw, minr)
